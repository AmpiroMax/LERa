...
if __name__ == '__main__':
    ...
    obs = env.reset(obj_list)
    from motion_primitives import *
    random.seed(42)
    observation = {"robot":[0,0,0]}
    #Initilize param
    drop_type = 'pick+place'
    drop_prob = [0.2, 1]
    motion = Motion_primitives(env, obj_list, drop_type=drop_type, drop_prob=drop_prob)
    
    start_time = time.time()
    # find the block
    observation["brown block"] = motion.locate("brown block")
    observation["pink block"] = motion.locate("pink block")
    print(f'find time: {time.time() - start_time}')

    # pick the block
    motion.pick(observation["brown block"])
    # motion.pick(observation["pink block"])
    print(f'pick time: {time.time() - start_time}')

    observation["brown bowl"] = motion.locate("brown bowl")
    print('-----------')
    print(observation)
    print(env.get_ee_pos())

    # motion.place(observation["brown bowl"])
    ...
    plt.imshow(img)
    # plt.imsave('camera_image_matplotlib.png', img)
    plt.show()
