...
https://github.com/alfworld/alfworld/blob/master/scripts/augment_trajectories.py
...
# import fiqa.perceivers.maskrcnn.alfworld_constants as constants
from alfred_utils.gen.utils.py_util import walklevel
...
def get_openable_points(traj_data):
    ...
    openable_json_file = os.path.join(
        'alfred_utils/gen/layouts/FloorPlan%d-openable.json' % scene_num
    )
...
def augment_traj(env, json_file):
    ...
    for ll_idx, ll_action in enumerate(traj_data['plan']['low_actions']):
        ...
        wanted_keys = [
            'action', 'objectId', 'receptacleObjectId', 'placeStationary',
            'forceAction'
        ]
        cmd = {k: cmd[k] for k in wanted_keys if k in cmd}
...
parser.add_argument(
    '--data_path', type=str, default='alfred_utils/data/json_2.1.0/train'
)
parser.add_argument(
    '--save_path', type=str, default='alfred_utils/data/detector'
)
...
parser.add_argument(
    '--reward_config', type=str,
    default='alfred_utils/models/config/rewards.json'
)
...
restricter = set()  # needed to filter out repeats of gt executions of episodes
for dir_name, subdir_list, file_list in walklevel(args.data_path, level=2):
    if 'trial_' in dir_name:
        task_desc = dir_name.split('/')[-2]
        if task_desc not in restricter:
            restricter.add(task_desc)
            json_file = os.path.join(dir_name, TRAJ_DATA_JSON_FILENAME)
            if (
                not os.path.isfile(json_file)
                or json_file in finished_jsons['finished']
            ):
                continue
