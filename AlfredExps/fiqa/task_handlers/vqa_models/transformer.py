...
"""
NB: The original file was modified by adding comments and by satisfying
code style.
...
class Transformer(nn.Module):
    def __init__(
        ...
        # Attention: ResNet is not cached in that directory...
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            if self.CLS is not None:
                # We add a CLS token to the image, to be used for
                # contrastive loss
                ...
                # Adding zeros as the first token in the sequence
                # to be compatible with the CLS token
                pos_embed = torch.cat(
                    (torch.zeros(1, bs, self.d_model, device=device), pos_embed)
                )

                # Adding one mask item to the beginning of the mask
                # to be compatible with CLS token
            ...
            if self.pass_pos_and_query:
                ...
                src, tgt, query_embed, pos_embed = \
                    src + 0.1 * pos_embed, query_embed, None, None

            device = src.device
            if isinstance(text[0], str):
                # Encode the text
                # tokenized --- dict with 2 fields:
                # "input_inds": shape = (b, longest + 2);
                # "attention_mask": shape = (b, longest + 2) (paddings are 0)
                tokenized = self.tokenizer.batch_encode_plus(
                    text, padding="longest", return_tensors="pt"
                ).to(device)
                # encoded_text --- BaseModelOutputWithPoolingAndCrossAttentions
                # with 2 fields: last_hidden_state, pooler_output
                encoded_text = self.text_encoder(**tokenized)

                # Transpose memory
                # because pytorch's attention expects sequence first
                ...
                # Resize the encoder hidden states
                # to be of the same d_model as the decoder
            ...
            # Pad the pos_embed with 0 so that
            # the addition will be a no-op for the text tokens
            ...
            memory_cache = {
                ...
                "text_pooled_op":
                    encoded_text.pooler_output if self.CLS is not None
                    else None,
                "img_pooled_op":  # Return the CLS token
                    img_memory[0] if self.CLS is not None else None,
                "mask": mask,
            ...
            # hs.transpose(1, 2).shape =
            # = (num_layers=6, b, num_queries + nb_heads, 256)
...
class TransformerDecoderLayer(nn.Module):
    ...
    # For now, trying one version where its self attn -> cross attn text ->
    # -> cross attn image -> FFN
    def forward_post(
        self,
        tgt,
        memory,
...
class FeatureResizer(nn.Module):
    """
    This class takes as input a set of embeddings of dimension C1
    and outputs a set of embedding of dimension C2,
    after a linear transformation, dropout and normalization (LN).
    """
