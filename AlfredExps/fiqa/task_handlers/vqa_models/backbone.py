...
from fiqa.task_handlers.vqa_models.util.misc import NestedTensor

from fiqa.task_handlers.vqa_models.position_encoding \
    import build_position_encoding
...
class TimmBackbone(nn.Module):
    def __init__(
        ...
        backbone = create_model(
            name, pretrained=True, in_chans=3, features_only=True,
            out_indices=(1, 2, 3, 4)
        )
...
def build_backbone(args):
    ...
    if args.backbone[: len("timm_")] == "timm_":
        ...
        backbone = GroupNormBackbone(
            args.backbone, train_backbone, return_interm_layers,
            args.dilation
        )
    else:
        backbone = Backbone(
