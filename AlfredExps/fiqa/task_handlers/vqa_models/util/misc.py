...
def collate_fn(do_round, batch):
    ...
    if "positive_map" in batch[1][0]:
        # We batch the positive maps here. Since in general each batch element
        # will have a different number of boxes,
        # we collapse a single batch dimension to avoid padding.
        ...
        # assert batched_pos_map.sum().item() == \
        #        sum([v["positive_map"].sum().item() for v in batch[1]])
...
class NestedTensor(object):
    ...
    def to(self, *args, **kwargs):
        ...
        cast_mask = \
            self.mask.to(*args, **kwargs) if self.mask is not None else None
    ...
    def from_tensor_list(cls, tensor_list, do_round=False):
        ...
        if tensor_list[0].ndim == 3:
            ...
            max_size = tuple(
                max(s) for s in zip(*[img.shape for img in tensor_list])
            )
            # min_size = tuple(
            #     min(s) for s in zip(*[img.shape for img in tensor_list])
            # )
...
def interpolate(
    input: Tensor, size: Optional[List[int]] = None,
    scale_factor: Optional[float] = None, mode: str = "nearest",
    align_corners: Optional[bool] = None
) -> Tensor:
    """
    Equivalent to nn.functional.interpolate,
    but with support for empty channel sizes.
    """
    if input.numel() > 0:
...
def targets_to(targets: List[Dict[str, Any]], device):
    ...
    return [{
        ...
        for k, v in t.items() if k != "caption"
    } for t in targets]
