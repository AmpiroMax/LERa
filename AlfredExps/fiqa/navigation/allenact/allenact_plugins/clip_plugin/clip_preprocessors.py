...
https://github.com/allenai/allenact/blob/main/allenact_plugins/clip_plugin/clip_preprocessors.py
"""
...
from fiqa.navigation.allenact.allenact.base_abstractions.preprocessor \
    import Preprocessor
from fiqa.navigation.allenact.allenact.utils.misc_utils \
    import prepare_locals_for_super
    ...
    def __init__(self, resnet: CLIP, pool=True, pooling_type="avg"):
        ...
        if not pool:
            ...
            raise NotImplementedError(
                "`pooling_type` must be 'avg' or 'attn'.")
...
class ClipResNetPreprocessor(Preprocessor):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        output_height_width = tuple(
            iis // 32 for iis in input_img_height_width)
    ...
    def resnet(self) -> ClipResNetEmbedder:
        if self._resnet is None:
            self._resnet = ClipResNetEmbedder(
                clip.load(self.clip_model_type, device=self.device)[
                    0], pool=self.pool
            ).to(self.device)
    ...
    def process(self, obs: Dict[str, Any], *args: Any, **kwargs: Any) -> Any:
        x = obs[self.input_uuids[0]].to(
            self.device).permute(0, 3, 1, 2)  # bhwc -> bchw
