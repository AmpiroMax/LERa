...
import fiqa.navigation.film.segmentation_definitions as segdef
...
class DepthEstimate():
    ...
    def __init__(self, depth_pred, num_bins, max_depth):
        ...
        if ((depth_pred.sum(dim=1) - 1).abs() < 1e-2).all():
            ...
            ), "Depth prediction needs to be a simplex at each pixel, current sum is " + str(depth_pred.sum(dim=1))
    ...
    def domain_image(self, res=None):
        ...
        domain_image = domain.repeat(
            (1, 1, self.depth_pred.shape[2], self.depth_pred.shape[3])) * (self.max_depth / res)
        return domain_image

    def mle(self):
        mle_depth = self.depth_pred.argmax(
            dim=1, keepdim=True).float() * (self.max_depth / self.num_bins)
        return mle_depth

    def expectation(self):
        expected_depth = (self.domain() * self.depth_pred).sum(dim=1,
                                                               keepdims=True) * (self.max_depth / self.num_bins)
...
class SimpleUNEt(torch.nn.Module):
    def __init__(self, distr_depth, vec_head, segonly):
        ...
        self.conv1 = ConvOp(self.p.in_channels, self.p.hc1,
                            3, stride=self.p.stride, padding=1)
        self.conv2 = ConvOp(self.p.hc1, self.p.hc1, 3,
                            stride=self.p.stride, padding=1)
        self.conv3 = ConvOp(self.p.hc1, self.p.hc1, 3,
                            stride=self.p.stride, padding=1)
        self.conv4 = ConvOp(self.p.hc1, self.p.hc1, 3,
                            stride=self.p.stride, padding=1)
        self.conv5 = ConvOp(self.p.hc1, self.p.hc1, 3,
                            stride=self.p.stride, padding=1)
        self.conv6 = ConvOp(self.p.hc1, self.p.hc1, 3,
                            stride=self.p.stride, padding=1)

        self.deconv1 = DeconvOp(self.p.hc1, self.p.hc1,
                                3, stride=self.p.stride, padding=1)
        ...
        self.deconv6 = DeconvOp(
            self.p.hc1 + self.p.hc2, self.p.out_channels, 3, stride=self.p.stride, padding=1)
    ...
    def forward(self, input):
        ...
        if self.vec_head:
            vec_head = self.linear2(
                self.act(self.linear1(x6.mean(dim=2).mean(dim=2))))
...
class AlfredSegmentationAndDepthModel(nn.Module):
    ...
    def __init__(self, hparams=None, distr_depth=DISTR_DEPTH, vec_head=VEC_HEAD, segonly=False):
        ...
        self.iter = nn.Parameter(torch.zeros(
            [1], dtype=torch.double), requires_grad=False)
    ...
    def predict(self, rgb_image):
        with torch.no_grad():
            if self.distr_depth:
                ...
                seg_pred = seg_pred / \
                    (seg_pred.sum(dim=1, keepdims=True) + 1e-10)
            else:
                seg_pred, depth_pred, vec_head = self.forward_model(rgb_image)
                seg_pred = torch.exp(seg_pred)

                good_seg_mask = seg_pred > 0.3
                good_depth_mask = (seg_pred > 0.5).sum(
                    dim=1, keepdims=True) * (depth_pred > 0.9)
                seg_pred = seg_pred * good_seg_mask
                seg_pred = seg_pred / \
                    (seg_pred.sum(dim=1, keepdims=True) + 1e-10)
    ...
    def forward(self, batch: Dict):
        ...
        if self.distr_depth:
            depth_flat_pred = depth_pred.permute(
                (0, 2, 3, 1)).reshape([b * h * w, DEPTH_BINS])
            depth_flat_gt = depth_gt.permute((0, 2, 3, 1)).reshape([b * h * w])
            depth_flat_gt = (
                (depth_flat_gt / DEPTH_MAX).clamp(0, 0.999) * DEPTH_BINS).long()
            depth_loss = self.nllloss(depth_flat_pred, depth_flat_gt)

            depth_pred_mean = (torch.arange(0, DEPTH_BINS, 1, device=depth_pred.device)[
                               None, :, None, None] * torch.exp(depth_pred)).sum(dim=1)
            depth_mae = (depth_pred_mean.view(
                [-1]) - depth_flat_gt).abs().float().mean() * (DEPTH_MAX / DEPTH_BINS)
