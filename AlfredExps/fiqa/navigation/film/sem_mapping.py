...
# from utils_f.distributions import Categorical, DiagGaussian
from fiqa.navigation.film.utils_f.model import get_grid, ChannelPool
import fiqa.navigation.film.envs.utils.depth_utils as du
...
class SemanticMapping(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.map_cells = self.map_size_cm//args.map_resolution

        self.last_map = torch.zeros([5, 240, 240])
    ...
    def forward(self, obs, pose_obs, maps_last, poses_last, build_maps=True, no_update=False):
        ...
        self.last_map = map_pred

        return fp_map_pred, map_pred, pose_pred, current_poses

    def world_to_map(self, x, y):
        i = int(y / self.args.map_resolution /
                0.01 - 0.9999) + self.map_cells // 2
        j = int(x / self.args.map_resolution / 0.01) + self.map_cells // 2
