...
class FMMPlanner():
    ...
    def set_multi_goal(self, goal_map):
        ...
        #     img_idx = len(glob.glob('fmm_dist_weird/*.png'))
        #     cv2.imwrite("fmm_dist_weird/" + "fmm_dist_" +
        #                 str(img_idx) + ".png", self.fmm_dist[:, ::-1])
        #     # self.save_t += 1
        return
    ...
    def get_short_term_goal(self, state, found_goal=0, decrease_stop_cond=0):
        ...
        stop_condition = max(
            (self.args.stop_cond - decrease_stop_cond)*100/5., 0.2)
        if not(found_goal):
