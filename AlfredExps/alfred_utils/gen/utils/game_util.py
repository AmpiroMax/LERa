...
https://github.com/askforalfred/alfred/blob/master/gen/utils/game_util.py
"""
import copy
import random
import cv2
...
import alfred_utils.gen.goal_library as glib
...
def get_obj_of_type_closest_to_obj(
    object_type, ref_object_id, metadata, action = None
):
    if action is not None and action.get('forceVisible'):
        objs_of_type = [
            obj for obj in metadata['objects'] if obj['objectType'] == object_type
        ]
    else:
        objs_of_type = [
            obj for obj in metadata['objects']
