...
https://github.com/askforalfred/alfred/blob/master/env/thor_env.py
and slightly changed (teleport support was added, see lines 463-464).
"""
import cv2
import copy
        ...
        if event.metadata['lastActionSuccess']:
            ...
            if action['action'] == 'ToggleObjectOn' and "Faucet" in action['objectId']:
                ...
                sink_basin = get_obj_of_type_closest_to_obj(
                    'SinkBasin', action['objectId'], event.metadata, action
                )
    ...
    def to_thor_api_exec(self, action, object_id="", smooth_nav=False):
        ...
        if "RotateLeft" in action:
            ...
            action = dict(action="SliceObject",
                          objectId=object_id)
            event = self.step(action)
        elif "action" in action and "TeleportFull" == action["action"]:
            event = self.step(action)
        else:
    ...
    def check_clean(self, action):
        ...
        if event.metadata['lastActionSuccess'] and 'Faucet' in object_id:
            ...
            sink_basin_obj = game_util.get_obj_of_type_closest_to_obj(
                "SinkBasin", object_id, event.metadata, action
            )
