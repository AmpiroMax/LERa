...
https://github.com/askforalfred/alfred/blob/master/env/reward.py
"""
from alfred_utils.gen.utils.game_util import get_object
...
class GotoLocationAction(BaseAction):
    ...
    valid_actions = {'MoveAhead', 'RotateLeft', 'RotateRight', 'LookUp',
                     'LookDown', 'Teleport', 'TeleportFull'}
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        reward = (
                             prev_distance - curr_distance) * 0.2  # distance reward factor?
...
class PickupObjectAction(BaseAction):
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if len(inventory_objects):
            ...
            reward, done = (self.rewards['positive'],
                            True) if inv_object_id == goal_object_id else (
            self.rewards['negative'], False)
        return reward, done
...
class PutObjectAction(BaseAction):
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if recep_object is not None:
            is_target_in_recep = target_object_id in recep_object[
                'receptacleObjectIds']
            reward, done = (
            self.rewards['positive'], True) if is_target_in_recep else (
            self.rewards['negative'], False)
        return reward, done
...
class OpenObjectAction(BaseAction):
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if target_recep is not None:
            ...
            reward, done = (
            self.rewards['positive'], True) if is_target_open else (
            self.rewards['negative'], False)
        return reward, done
...
class CloseObjectAction(BaseAction):
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if target_recep is not None:
            ...
            reward, done = (
            self.rewards['positive'], True) if is_target_closed else (
            self.rewards['negative'], False)
        return reward, done
...
class ToggleObjectAction(BaseAction):
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if target_toggle is not None:
            ...
            reward, done = (
            self.rewards['positive'], True) if is_target_toggled else (
            self.rewards['negative'], False)
        return reward, done
...
class SliceObjectAction(BaseAction):
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if target_object is not None:
            ...
            reward, done = (
            self.rewards['positive'], True) if is_target_sliced else (
            self.rewards['negative'], False)
        return reward, done
...
class CleanObjectAction(BaseAction):
    ...
    valid_actions = {'PutObject', 'PickupObject', 'ToggleObjectOn',
                     'ToggleObjectOff'}
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if clean_object is not None:
            ...
            reward, done = (
            self.rewards['positive'], True) if is_obj_clean else (
            self.rewards['negative'], False)
        return reward, done
...
class HeatObjectAction(BaseAction):
    ...
    def get_reward(self, state, prev_state, expert_plan, goal_idx):
        ...
        if next_put_goal_idx < len(expert_plan):
            ...
            reward, done = (self.rewards['positive'], True) if is_obj_hot else (
            self.rewards['negative'], False)
        return reward, done
